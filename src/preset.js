...
export const globalInit = async () => {
  const ajax = createAjax({
    baseURL: baseApiUrl, errorHandler: error => message.error(error), getDefaultHeaders: () => {
      return {
        env, appName, Authorization: `Bearer ${cookie.get('token')}`
      };
  ...
  const registry = {
    url: cdnHost, tpl: '{{url}}/components/@kne-components/{{remote}}/{{version}}/build'
  };

  const componentsCoreRemote = {
    ...registry, //url: 'http://localhost:3001',
    //tpl: '{{url}}',
    remote: 'components-core', defaultVersion: '0.3.24'
  };
  remoteLoaderPreset({
    remotes: {
      default: componentsCoreRemote, 'components-core': componentsCoreRemote, 'components-iconfont': {
        ...
        //tpl: '{{url}}',
        remote: 'components-ckeditor', defaultVersion: '0.2.5'
      }, 'leapin-ai-agent': process.env.NODE_ENV === 'development' ? {
        remote: 'leapin-ai-agent', url: '/', tpl: '{{url}}', defaultVersion: process.env.DEFAULT_VERSION
      } : {
        remote: 'leapin-ai-agent', url: 'https://app.ap.leapin-ai.com', tpl: '{{url}}/ai-agent'
      }
    }
  });

  return {
    ajax, appName, env, conferenceHost, enums: Object.assign({}, {
      atsStage: [{ description: 'Primary screening', value: 0 }, {
        description: 'AI interview', value: 1
      }, { description: 'Retest', value: 3 }, {
        description: 'Awaiting entry', value: 4
      }, { description: 'Inappropriate', value: 5 }, { description: 'Offer', value: 8 }, {
      ...
      agent: getAgentApis(), file: {
        ...
        upload: async ({ file }) => {
          ...
          const { data: resData } = await ajax(Object.assign({}, {
            url: '/api/common/upload/token', params: { media_params: 'candidate-cv' }, method: 'GET'
          }));
          if (resData.code !== 0) {
          ...
          const { data: uploadRes } = await ajax.postForm(Object.assign({}, {
            url: ossConfig.host, showError: false, data: {
          ...
          return {
            data: {
              code: 0, data: { src: xmlDoc.getElementsByTagName('Location')[0].textContent, filename: file.name }
            }
          };
        }
      }
    }), locale: 'en-US', themeToken: {
      colorPrimary: '#2257bf'
    }, formInfo: {
      type: 'default'
    }
